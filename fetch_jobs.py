...
def load_seen_jobs():
    """loads all historical job hashes into a set for fast lookup."""
...
def save_seen_jobs(updated_set):
    """saves the growing database, sorted for clean git history."""
    if not updated_set:
        return
...
def scrape_telegram():
    """fetches from sarkariexam_info channel"""
    jobs = []
...
def main():
    ...
    if new_jobs_list:
        ...
        # 5. update growing history file
        save_seen_jobs(history_set)
    else:
        print("logs: no new jobs found.")
